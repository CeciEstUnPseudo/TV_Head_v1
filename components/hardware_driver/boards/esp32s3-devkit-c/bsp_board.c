 ...
 */
 ...
 // This is for ESP32-S3-DevKitC board specific configuration.
#include "string.h"
#include "bsp_board.h"
...
static i2s_chan_handle_t rx_handle = NULL;        // I2S rx channel handler
    ...
    /**
     * @brief = Initialize I2S interface for audio data reading and shift to 16 bits to match int16_t buffer
     * Also supports getting raw channel data. Shifts data to 16 bits after reading cause ADC data is 18 bits and I2S read is 32 bits so we need to right shift by 14 bits (32->18->16)
     * 
     * @param i2s_num I2S port number to initialize
     * @param sample_rate Sample rate for I2S interface
     * @param channel_format Number of channels for I2S interface
     * @param bits_per_chan Bits per channel for I2S interface
     * 
     * @return
     *    - ESP_OK: Success
     *    - Others: Fail
...
static esp_err_t bsp_i2s_init(i2s_port_t i2s_num, uint32_t sample_rate, int channel_format, int bits_per_chan)
{
    ...
    /**
     * @brief Deinitialize I2S interface because only need to use I2S for audio input (only record function + ADC is used) basically to free up resources
     * 
     * @param i2s_num I2S port number to deinitialize
     * 
     * @return
     *    - ESP_OK: Success
     *    - Others: Fail
...
static esp_err_t bsp_i2s_deinit(i2s_port_t i2s_num)
{
    ...
    /**
     * @brief Read audio data from I2S interface and shift to 16 bits to match int16_t buffer
     * Also supports getting raw channel data if needed
     * 
     * @param is_get_raw_channel Set to true to get raw channel data without any shifting
...
esp_err_t bsp_get_feed_data(bool is_get_raw_channel, int16_t *buffer, int buffer_len)
{
    ...
    /**
     * @brief Get the number of channels used for audio data reading
     * 
     * @return 
     *   - Number of channels (DUH)
     */
...
/**
 * @brief Special config for dev board, basically init I2S for audio input
 * 
 * @param sample_rate Sample rate for I2S interface
 * @param channel_format Number of channels for I2S interface
 * @param bits_per_chan Bits per channel for I2S interface
...
/**
 * @brief Initialize SD card and mount FAT filesystem (FAT = File Allocation Table btw)
 * 
 * @param mount_point Mount point of the SD card
...
/**
 * @brief Unmount SD card from FAT filesystem and release resources acquired
 * 
 * @param mount_point Mount point of the SD card
 * 
 * @return
 *    - ESP_OK                  Success
 *    - ESP_ERR_INVALID_STATE   If SD card was not initialized
 *    - Others                  Fail
 */
